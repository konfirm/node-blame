...
function BlameResult(message, trace, steps) {
	var result = this,
		util = require('./util'),
		toString = '{@message:Message: {message}@}{%trace:\n\t* {@call:{call} [@}{file} @{line}:{column}{@call:]@}%}';
	...
	function init() {
		...
		Object.defineProperties(result, {
			//  message and trace are exposed to the public as being members
			message: util.define(false, message),
			trace: util.define(false, trace),
	...
	/**
	 *  Filter the trace array to contain only matching items
	 *  @name    matches
	 *  @access  internal
	 *  @param   BlameItem item
	...
	function filter(pattern) {
		...
		return trace.filter(function(item) {
