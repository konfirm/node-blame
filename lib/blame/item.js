...
function BlameItem(message, data) {
	 ...
	 */
	item.context = polymorphic();

	//  template, before, after
	item.context.signature('string template, number before=3, number after=0', function(tpl, before, after) {
		return util.template(tpl, context(before, after || before));
	});

	//  before, after
	item.context.signature('number before=3, number after=0', function(before, after) {
		return util.template(template.context, context(before, after || before));
	});
