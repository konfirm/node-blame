...
function Blame() {
	...
	function stack(error) {
		...
		return new BlameResult(
			...
			lines.map(function(line, index) {
				...
				return match ? new BlameItem(message, {file: file || match[1], line: +match[2], column: +match[3], call: call, index: index - 1, order: lines.length - index}) : null;
			})
	...
	blame.trace = nodeUtil.deprecate(blame.stack, 'blame.trace is deprecated: use blame.stack instead');
}
